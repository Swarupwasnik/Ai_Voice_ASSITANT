document.addEventListener("DOMContentLoaded", function () {
        ...
        async function fetchJson(url, options = {}) {
          ...
          if (!ct.toLowerCase().includes("application/json")) {
            // Read text and throw a descriptive error so we can see HTML/ngrok pages
        ...
        countryCodeSelect.addEventListener("change", function() {
          ...
          if (this.value === '+91') {
            ...
            mobile.maxLength = 10;
            mobileValidation.textContent = "Type only 10 digit Indian mobile no";
            mobileValidation.className = "mobile-validation info";
          } else if (this.value === 'Int.') {
            mobile.placeholder = "Mobile with Code";
            mobile.maxLength = 15;
            mobileValidation.textContent = "Mobile no type with country code";
            mobileValidation.className = "mobile-validation info";
          }
          mobile.value = "";
          updateStartButton();
        ...
        mobileInput.addEventListener("input", function () {
          ...
          if (this.value.trim() === "") {
            ...
            mobileValidation.textContent = countryCode === '+91' ? "✓ Verified Indian no" : "✓ Verified international no";
        ...
        startCallBtn.addEventListener("click", async function () {
          ...
          // Show connecting modal with dynamic message
          ...
          // Determine phone number and agent phone number ID
          let to_number;
          ...
          // Select phone number ID based on country code - use international for both to avoid Twilio number issues
          const agent_phone_number_id = INTERNATIONAL_PHONE_NUMBER_ID;

          // Determine region selection based on country code
          let region_selection = "india";
          if (countryCode === 'Int.') {
            // For international numbers, try to detect region or default to US
            if (to_number.startsWith('+1')) {
              ...
              region_selection = "us"; // Default for international
            }
          }
          ...
          try {
            ...
            // Update modal status and chat header
        ...
        async function loadConversation(conversationId) {
          try {
            ...
            // Add call end message after all transcript messages
            setTimeout(() => {
              const nameForMsg = lastCustomerName || "User";
              addMessage(`${nameForMsg} cut the call.`, "sender");
            }, (transcript.length + 1) * 500);
          } catch (err) {
        ...
        // Fetch and render call summary into the Call Summary card
        ...
        function onCallEnded(trigger = "user") {
          ...
          // Force close modal and show chat
          modal.classList.remove("active");
          chatDashboard.style.display = "flex";
          document.querySelector(".form-section").style.display = "none";
        ...
        function startAutoDetectConversationEnd() {
          ...
          async function checkEnd() {
            try {
              ...
              // Look for any conversation newer than call start
              ...
              if (chosen && chosen.conversation_id && chosen.conversation_id !== lastConversationId) {
                ...
                // Force close modal and show conversation
                modal.classList.remove("active");
                chatDashboard.style.display = "flex";
                document.querySelector(".form-section").style.display = "none";
        ...
        function pollForConversation() {
          ...
          async function check() {
            try {
              ...
              // Find any conversation newer than call start
              const eligible = items.filter((c) => {
                if (!c || !c.timestamp) return false;
                const ts = Date.parse(c.timestamp);
                return !Number.isNaN(ts) && ts >= startedAtMs;
          ...
          // Start polling immediately, then every 2s for faster response
          check();
